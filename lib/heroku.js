...
var Heroku = (module.exports = (function () {
  function Heroku(token) {
    ...
    const hrku = require("heroku-client");

    this.client = new hrku({ token: this.token });
  }
  ...
  Heroku.prototype.clone = function (repo_url, app_name) {
    ...
    if (!isURL(repo_url)) {
      this.emit(
        "error",
        "repo_url must be a fully-qualified URL or shorthand string like 'github:user/repo' or 'bitbucket:user/repo#branch'"
      );
      return;
    }
  ...
  Heroku.prototype.deriveAddonsAndEnv = function (app_name, cb) {
    ...
    this.client.get("/apps/" + app_name + "/addons", function (err, addons) {
      ...
      var configVarsCreatedByAddons = addons
        .map(function (addon) {
  ...
  Heroku.prototype.deriveAppNamesFromLocalGitConfig = function (cb) {
    if (!fs.existsSync(process.cwd() + "/.git/config")) return cb(null, []);

    var config = ini.parse(
      fs.readFileSync(process.cwd() + "/.git/config").toString()
    );
